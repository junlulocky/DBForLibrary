...
function calendar(objName,name,date,width,height){
	...
	this.dayName=['Sum','Mon','Tue','Wed','Thu','Fri','Sat'];
	...
	function getTable(){
		...
		var openurl = '<td id="'+this.calendarName+i+'">'+'<a href="http://www.w3school.com.cn/php/'+this.channel+'/'+this.dateTime.getFullYear()+monthValue+'/'+dayValue+'_1.htm" target="_blank">'+i+'</a></td>';
		var ropenurl = '<td id="'+this.calendarName+i+'">'+'<a href="http://www.w3school.com.cn/php/'+this.channel+'/'+this.dateTime.getFullYear()+monthValue+'/'+dayValue+'_1.htm" target="_blank"><font color="red">'+i+'</font></a></td>';
		...
		{
					this.dateTable+='<td><button hidefocus="true"><span style="font-family:Webdings">3</span></button></td>'
			+'<td colspan="2">'+this.dateTime.getFullYear().toString().bold()+'</td>'
			+'<td>'+(this.dateTime.getMonth()+1).toString().bold()+'</td>'
			+'<td><button onmouseup="void('+this.objName+'.nextMonth())" hidefocus="true"><span style="font-family:Webdings">4</span></button></td>'
			+'</tr></table>';
		}
		else
		{
		this.dateTable+='<td><button onmouseup="void('+this.objName+'.previewMonth())" hidefocus="true"><span style="font-family:Webdings">3</span></button></td>'
			+'<td colspan="2">'+this.dateTime.getFullYear().toString().bold()+'</td>'
			+'<td>'+(this.dateTime.getMonth()+1).toString().bold()+'</td>'
			+'<td><button onmouseup="void('+this.objName+'.nextMonth())" hidefocus="true"><span style="font-family:Webdings">4</span></button></td>'
			+'</tr></table>';
